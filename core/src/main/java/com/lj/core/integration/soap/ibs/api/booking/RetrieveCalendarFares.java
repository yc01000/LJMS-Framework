...
import com.lj.core.integration.soap.ibs.domain.booking.CalendarRetrieveFaresRQ;
import com.lj.core.integration.soap.ibs.domain.booking.CalendarRetrieveFaresRS;
import com.lj.core.integration.soap.ibs.domain.booking.ErrorType;

@Component("RetrieveCalendarFares")
public class RetrieveCalendarFares extends IbsSoapConnector<CalendarRetrieveFaresRQ, CalendarRetrieveFaresRS, IbsSoapProperty>{
	
	@Value("#{IbsBookingProperties['booking.availability.endpoint']}")
	private String endpoint;
	
	@Value("#{IbsBookingProperties['booking.availability.retrieveCalendarFares']}")
	private String action;
	
	@Resource(name="webServiceTemplate.booking")
	private WebServiceTemplate webServiceTemplate;

	public CalendarRetrieveFaresRS request(CalendarRetrieveFaresRQ req, IbsSoapProperty property){
		initiate(endpoint, action);
		CalendarRetrieveFaresRS response = null;
		try{
			super.setWebServiceTemplate(webServiceTemplate);
			response = super.request(req, property);
		}catch(SoapFaultException sfe){			
			response = new CalendarRetrieveFaresRS();
