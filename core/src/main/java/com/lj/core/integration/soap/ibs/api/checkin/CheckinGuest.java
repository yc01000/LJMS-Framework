...
import com.lj.core.integration.soap.ibs.domain.checkin.CHKCheckinGuestRQ;
import com.lj.core.integration.soap.ibs.domain.checkin.CHKCheckinGuestRS;

@Component("CheckinGuest")
public class CheckinGuest extends IbsSoapConnector<CHKCheckinGuestRQ, CHKCheckinGuestRS, IbsSoapProperty>{
	
	@Value("#{IbsBookingProperties['checkin.checkin.endpoint']}")
	private String endpoint;
	
	@Value("#{IbsBookingProperties['checkin.checkin.checkinGuest']}")
	...
	public CHKCheckinGuestRS request(CHKCheckinGuestRQ req, IbsSoapProperty property){
		initiate(endpoint, action);
		CHKCheckinGuestRS response = null;
		try{
			super.setWebServiceTemplate(webServiceTemplate);
			response = super.request(req, property);
		}catch(SoapFaultException sfe){			
			response = new CHKCheckinGuestRS();
