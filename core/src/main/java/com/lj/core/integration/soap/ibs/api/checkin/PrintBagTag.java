...
import com.lj.core.integration.soap.ibs.domain.checkin.CHKPrintBagTagRQ;
import com.lj.core.integration.soap.ibs.domain.checkin.CHKPrintBagTagRS;
...
@Component("PrintBagTag")
public class PrintBagTag extends IbsSoapConnector<CHKPrintBagTagRQ, CHKPrintBagTagRS, IbsSoapProperty>{
	
	@Value("#{IbsBookingProperties['checkin.checkin.endpoint']}")
	private String endpoint;
	
	@Value("#{IbsBookingProperties['checkin.checkin.printBagTag']}")
	...
	public CHKPrintBagTagRS request(CHKPrintBagTagRQ req, IbsSoapProperty property){
		initiate(endpoint, action);
		CHKPrintBagTagRS response = null;
		try{
			super.setWebServiceTemplate(webServiceTemplate);
			response = super.request(req, property);
		}catch(SoapFaultException sfe){			
			response = new CHKPrintBagTagRS();
