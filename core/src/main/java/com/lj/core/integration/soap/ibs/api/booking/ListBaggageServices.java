...
import com.lj.core.integration.soap.ibs.domain.booking.ListBaggageServicesRQ;
import com.lj.core.integration.soap.ibs.domain.booking.ListBaggageServicesRS;

@Component("ListBaggageServices")
public class ListBaggageServices extends IbsSoapConnector<ListBaggageServicesRQ, ListBaggageServicesRS, IbsSoapProperty>{
	
	@Value("#{IbsBookingProperties['booking.ancillary.endpoint']}")
	private String endpoint;
	
	@Value("#{IbsBookingProperties['booking.ancillary.listBaggageServices']}")
	private String action;
	
	@Resource(name="webServiceTemplate.booking")
	private WebServiceTemplate webServiceTemplate;
	
	public ListBaggageServicesRS request(ListBaggageServicesRQ req, IbsSoapProperty property){
		initiate(endpoint, action);
		ListBaggageServicesRS response = null;
		try{
			super.setWebServiceTemplate(webServiceTemplate);
			response = super.request(req, property);
		}catch(SoapFaultException sfe){			
			response = new ListBaggageServicesRS();
