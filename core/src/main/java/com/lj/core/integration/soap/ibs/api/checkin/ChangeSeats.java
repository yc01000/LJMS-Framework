...
import com.lj.core.integration.soap.ibs.domain.checkin.CHKChangeSeatsRQ;
import com.lj.core.integration.soap.ibs.domain.checkin.CHKChangeSeatsRS;
import com.lj.core.integration.soap.ibs.domain.checkin.ErrorType;

@Component("ChangeSeats")
public class ChangeSeats extends IbsSoapConnector<CHKChangeSeatsRQ, CHKChangeSeatsRS, IbsSoapProperty>{
	
	@Value("#{IbsBookingProperties['checkin.checkin.endpoint']}")
	private String endpoint;
	
	@Value("#{IbsBookingProperties['checkin.checkin.changeSeats']}")
	private String action;
		
	@Resource(name="webServiceTemplate.checkin")
	private WebServiceTemplate webServiceTemplate;

	public CHKChangeSeatsRS request(CHKChangeSeatsRQ req, IbsSoapProperty property){
		initiate(endpoint, action);
		CHKChangeSeatsRS response = null;
		try{
			super.setWebServiceTemplate(webServiceTemplate);
			response = super.request(req, property);
		}catch(SoapFaultException sfe){			
			response = new CHKChangeSeatsRS();
