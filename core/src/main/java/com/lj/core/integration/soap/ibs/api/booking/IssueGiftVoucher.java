...
import com.lj.core.integration.soap.ibs.domain.booking.IssueGiftVoucherRQ;
import com.lj.core.integration.soap.ibs.domain.booking.IssueGiftVoucherRS;

//2017/09/25 사용하지 않는 웹서비스  정리 - by sunnymean
//@Component("IssueGiftVoucher")
public class IssueGiftVoucher extends IbsSoapConnector<IssueGiftVoucherRQ, IssueGiftVoucherRS, IbsSoapProperty>{
	
	@Value("#{IbsBookingProperties['booking.giftVoucher.endpoint']}")
	private String endpoint;
	
	@Value("#{IbsBookingProperties['booking.giftVoucher.issueGiftVoucher']}")
	private String action;
	
	@Resource(name="webServiceTemplate.booking")
	private WebServiceTemplate webServiceTemplate;

	public IssueGiftVoucherRS request(IssueGiftVoucherRQ req, IbsSoapProperty property){
		initiate(endpoint, action);
		IssueGiftVoucherRS response = null;
		try{
			super.setWebServiceTemplate(webServiceTemplate);
			response = super.request(req, property);
		}catch(SoapFaultException sfe){			
			response = new IssueGiftVoucherRS();
