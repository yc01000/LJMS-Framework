...
import com.lj.core.integration.soap.ibs.domain.booking.CreateTravelAgentRQ;
import com.lj.core.integration.soap.ibs.domain.booking.CreateTravelAgentRS;
import com.lj.core.integration.soap.ibs.domain.booking.ErrorType;

@Component("CreateTravelAgent")
public class CreateTravelAgent extends IbsSoapConnector<CreateTravelAgentRQ, CreateTravelAgentRS, IbsSoapProperty>{
	
	@Value("#{IbsBookingProperties['booking.agency.endpoint']}")
	private String endpoint;
	
	@Value("#{IbsBookingProperties['booking.agency.createTravelAgent']}")
	private String action;
	
	@Resource(name="webServiceTemplate.booking")
	private WebServiceTemplate webServiceTemplate;
	
	public CreateTravelAgentRS request(CreateTravelAgentRQ req, IbsSoapProperty property){
		initiate(endpoint, action);
		CreateTravelAgentRS response = null;
		try{
			super.setWebServiceTemplate(webServiceTemplate);
			response = super.request(req, property);
		}catch(SoapFaultException sfe){			
			response = new CreateTravelAgentRS();
