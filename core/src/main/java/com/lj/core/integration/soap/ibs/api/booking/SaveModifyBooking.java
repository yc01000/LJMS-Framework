...
import com.lj.core.integration.soap.ibs.domain.booking.SaveModifyBookingRQ;
import com.lj.core.integration.soap.ibs.domain.booking.SaveModifyBookingRS;

@Component("SaveModifyBooking")
public class SaveModifyBooking extends IbsSoapConnector<SaveModifyBookingRQ, SaveModifyBookingRS, IbsSoapProperty>{
	
	@Value("#{IbsBookingProperties['booking.reservations.endpoint']}")
	private String endpoint;
	
	@Value("#{IbsBookingProperties['booking.reservations.saveModifyBooking']}")
	private String action;
	
	@Resource(name="webServiceTemplate.booking")
	private WebServiceTemplate webServiceTemplate;

	public SaveModifyBookingRS request(SaveModifyBookingRQ req, IbsSoapProperty property){
		initiate(endpoint, action);
		SaveModifyBookingRS response = null;
		try{
			super.setWebServiceTemplate(webServiceTemplate);
			response = super.request(req, property);
		}catch(SoapFaultException sfe){			
			response = new SaveModifyBookingRS();
