...
import com.lj.core.integration.soap.ibs.domain.booking.AcceptWlRQ;
...
@Component("AcceptWl")
public class AcceptWl extends IbsSoapConnector<AcceptWlRQ, AcceptWlRS, IbsSoapProperty> {
	
	@Value("#{IbsBookingProperties['booking.reservations.endpoint']}")
	private String endpoint;
	
	@Value("#{IbsBookingProperties['booking.reservations.acceptWl']}")
	private String action;
	
	@Resource(name="webServiceTemplate.booking")
	private WebServiceTemplate webServiceTemplate;
	
	public AcceptWlRS request(AcceptWlRQ req, IbsSoapProperty property){
		initiate(endpoint, action);
		AcceptWlRS response = null;
		try{
			super.setWebServiceTemplate(webServiceTemplate);
			response = super.request(req, property);
		}catch(SoapFaultException sfe){			
			response = new AcceptWlRS();
