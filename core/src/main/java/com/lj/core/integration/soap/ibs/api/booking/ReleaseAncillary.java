...
import com.lj.core.integration.soap.ibs.domain.booking.ReleaseAncillaryRQ;
import com.lj.core.integration.soap.ibs.domain.booking.ReleaseAncillaryRS;

@Component("ReleaseAncillary")
public class ReleaseAncillary extends IbsSoapConnector<ReleaseAncillaryRQ, ReleaseAncillaryRS, IbsSoapProperty>{
	
	@Value("#{IbsBookingProperties['booking.reservations.endpoint']}")
	private String endpoint;
	
	@Value("#{IbsBookingProperties['booking.reservations.releaseAncillary']}")
	private String action;
	
	@Resource(name="webServiceTemplate.booking")
	private WebServiceTemplate webServiceTemplate;
	
	public ReleaseAncillaryRS request(ReleaseAncillaryRQ req, IbsSoapProperty property){
		initiate(endpoint, action);
		ReleaseAncillaryRS response = null;
		try{
			super.setWebServiceTemplate(webServiceTemplate);
			response = super.request(req, property);
		}catch(SoapFaultException sfe){			
			response = new ReleaseAncillaryRS();
