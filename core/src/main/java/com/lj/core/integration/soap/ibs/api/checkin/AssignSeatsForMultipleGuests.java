...
import com.lj.core.integration.soap.ibs.domain.checkin.CHKAssignSeatsForMultipleGuestsRQ;
import com.lj.core.integration.soap.ibs.domain.checkin.CHKAssignSeatsForMultipleGuestsRS;
import com.lj.core.integration.soap.ibs.domain.checkin.ErrorType;

@Component("AssignSeatsForMultipleGuests")
public class AssignSeatsForMultipleGuests extends IbsSoapConnector<CHKAssignSeatsForMultipleGuestsRQ, CHKAssignSeatsForMultipleGuestsRS, IbsSoapProperty>{

	@Value("#{IbsBookingProperties['checkin.checkin.endpoint']}")
	private String endpoint;
	
	@Value("#{IbsBookingProperties['checkin.checkin.assignSeatsForMultipleGuests']}")
	private String action;
		
	@Resource(name="webServiceTemplate.checkin")
	private WebServiceTemplate webServiceTemplate;

	public CHKAssignSeatsForMultipleGuestsRS request(CHKAssignSeatsForMultipleGuestsRQ req, IbsSoapProperty property){
		initiate(endpoint, action);
		CHKAssignSeatsForMultipleGuestsRS response = null;
		try{
			super.setWebServiceTemplate(webServiceTemplate);
			response = super.request(req, property);
		}catch(SoapFaultException sfe){			
			response = new CHKAssignSeatsForMultipleGuestsRS();
