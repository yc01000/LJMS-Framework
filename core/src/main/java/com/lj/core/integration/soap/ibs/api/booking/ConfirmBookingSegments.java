...
import com.lj.core.integration.soap.ibs.domain.booking.ConfirmBookingSegmentRQ;
...
@Component("ConfirmBookingSegments")
public class ConfirmBookingSegments extends IbsSoapConnector<ConfirmBookingSegmentRQ, ConfirmBookingSegmentRS, IbsSoapProperty>{
	
	@Value("#{IbsBookingProperties['booking.reservations.endpoint']}")
	private String endpoint;
	
	@Value("#{IbsBookingProperties['booking.reservations.confirmBookingSegments']}")
	private String action;
	
	@Resource(name="webServiceTemplate.booking")
	private WebServiceTemplate webServiceTemplate;
	
	public ConfirmBookingSegmentRS request(ConfirmBookingSegmentRQ req, IbsSoapProperty property){
		initiate(endpoint, action);
		ConfirmBookingSegmentRS response = null;
		try{
			super.setWebServiceTemplate(webServiceTemplate);
			response = super.request(req, property);
		}catch(SoapFaultException sfe){			
			response = new ConfirmBookingSegmentRS();
