...
import com.lj.core.integration.soap.ibs.domain.booking.RetrieveOandDPairsRQ;
import com.lj.core.integration.soap.ibs.domain.booking.RetrieveOandDPairsRS;

@Component("GetAllOandDPairs")
public class GetAllOandDPairs extends IbsSoapConnector<RetrieveOandDPairsRQ, RetrieveOandDPairsRS, IbsSoapProperty>{
	
	@Value("#{IbsBookingProperties['booking.availability.endpoint']}")
	private String endpoint;
	
	@Value("#{IbsBookingProperties['booking.availability.getAllOandDPairs']}")
	private String action;
	
	@Resource(name="webServiceTemplate.booking")
	private WebServiceTemplate webServiceTemplate;

	public RetrieveOandDPairsRS request(RetrieveOandDPairsRQ req, IbsSoapProperty property){
		initiate(endpoint, action);
		RetrieveOandDPairsRS response = null;
		try{
			super.setWebServiceTemplate(webServiceTemplate);
			response = super.request(req, property);
		}catch(SoapFaultException sfe){			
			response = new RetrieveOandDPairsRS();
