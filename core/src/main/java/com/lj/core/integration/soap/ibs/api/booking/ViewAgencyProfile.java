...
import com.lj.core.integration.soap.ibs.domain.booking.ViewAgencyProfileRQ;
import com.lj.core.integration.soap.ibs.domain.booking.ViewAgencyProfileRS;

@Component("ViewAgencyProfile")
public class ViewAgencyProfile extends IbsSoapConnector<ViewAgencyProfileRQ, ViewAgencyProfileRS, IbsSoapProperty>{
	
	@Value("#{IbsBookingProperties['booking.agency.endpoint']}")
	private String endpoint;
	
	@Value("#{IbsBookingProperties['booking.agency.viewAgencyProfile']}")
	private String action;
	
	@Resource(name="webServiceTemplate.booking")
	private WebServiceTemplate webServiceTemplate;

	public ViewAgencyProfileRS request(ViewAgencyProfileRQ req, IbsSoapProperty property){
		initiate(endpoint, action);
		ViewAgencyProfileRS response = null;
		try{
			super.setWebServiceTemplate(webServiceTemplate);
			response = super.request(req, property);
		}catch(SoapFaultException sfe){			
			response = new ViewAgencyProfileRS();
